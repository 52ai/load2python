...
Version 1.0: 增加了统计频次
Version 1.1: 把返回的执行序列号为0和-1的等待时间区分开来
             做相同带宽，都为100M
             //每隔一秒计算一次带宽利用率（不用改代码，直接读之前的表）
             写仿真分析报告的时候（要对参数设计对应的预估结果和实际结果作比较）
...
max_waiting_time = 200  # 最长等待时间间隔200s
per_time = 2  # 单次执行时间
...
以上参数，10min内（亦即600s），在服务器带宽1000M的情况下，对1000个客户端，最大可处理3轮
理想情况下，总的成功测试次数为3000次

实验结果：

图1，带宽利用率图（对横坐标和纵坐标标描述下）
图2，每个客户端成功测试次数图（对横坐标和纵坐标标描述下）
图3，每个客户端总的请求次数
...
def send_request(u_id, sum_online_time, sum_test_times, bandwidth):
    ...
    # 如果当前等待队列的长度大于等于max_queue_len，则表示队列已满，比较优先级，再判断是否入队，或者丢弃
        ...
        if pre_n < max_len_queue:
            ...
            # 修改下一次发起请求的时间
            client_table[int(del_u_id) - 1][4] = time.time() + (max_len_queue / 10) * per_time
            # 移除该任务
...
def send_confirm(u_id, sum_online_time, sum_test_times, bandwidth):
    ...
    if loc == -1:
        # 没有找到
        # 给客户端返回-1，其且修改下一次访问时间，当前队列执行完的最大时间
        # print "not find confirm task, u_id: %s" % u_id
        client_table[int(u_id)-1][5] = -1
        client_table[int(u_id)-1][4] = time.time() + (max_len_queue / 10) * per_time
    else:
